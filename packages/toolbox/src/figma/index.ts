...
class Figma {
  ...
  async deleteComment(file_key: string, comment_id: string): Promise<void> {
    return this._api.request('DELETE', `files/${file_key}/comments/${comment_id}`);
  }
  ...
  async getProjectFiles(
    ...
    return this._api.request('GET', `projects/${project_id}/files${queryString}`);
  }
  ...
  async getTeamComponentSets(
    ...
    return this._api.request('GET', `teams/${team_id}/component_sets${queryString}`);
  }

  async getFileComponentSets(file_key: string): Promise<GetFileComponentSetsResult> {
