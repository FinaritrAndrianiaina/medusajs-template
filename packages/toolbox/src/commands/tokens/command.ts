...
import { FIGMA_FILE_ID, FONT_FAMILY_MONO, FONT_FAMILY_SANS } from '../../constants';
import { client } from '../../figma-client';
import { logger } from '../../logger';
import { colorToRGBA, createLinearGradientComponent, gradientValues } from './utils/colors';
...
export async function generateTokens({ output }: GenerateTokensArgs) {
  ...
  await fse
    .outputFile(typoPath, `export const typography = ${JSON.stringify(typo, null, 2)}`)
    .then(() => {
  ...
  Object.values(colorStyles.nodes).reduce((acc, curr) => {
    ...
    if (solid && solid.color) {
      acc['colors'][lowerCaseTheme][solidVariable] = colorToRGBA(solid.color, solid.opacity);
    }
  ...
  await fse
    .outputFile(colorTokensPath, `export const colors = ${JSON.stringify(themeNode.colors, null, 2)}`)
    .then(() => {
  ...
  await fse
    .outputFile(effectTokensPath, `export const effects = ${JSON.stringify(themeNode.effects, null, 2)}`)
    .then(() => {
  ...
  await fse
    .outputFile(
      componentTokensPath,
      `export const components = ${JSON.stringify(themeNode.components, null, 2)}`
    )
    .then(() => {
  ...
  await fse
    .outputFile(tailwindConfigPath, `export const theme = ${JSON.stringify(themeExtension, null, 2)}`)
    .then(() => {
