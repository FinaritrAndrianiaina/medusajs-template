...
export async function generateIcons({ output }: GenerateIconsArgs) {
  ...
  for (let i = 0; i < length; i += 20) {
    ...
    const requests = slice.map(async (icon) => {
      ...
      const { componentName, fileName, testName, fixed } = getIconData(icon.name, icon.frame_name);
  ...
  if (skippedIcons.length) {
    logger.warn(`Skipped ${skippedIcons.length} icons. Check the logs for more information.`);
  }
